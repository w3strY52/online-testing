...
function MainTableMaterial(props: { id: number, isChecked: boolean }) {
    const res = testListMock.filter(x => x.disciplineId === props.id && x.checked === props.isChecked);
    return (
        <div>
            {!!res.length?
                <TableContainer sx={{maxHeight: 700}}>
                    <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody sx={{backgroundColor: 'rgba(190, 220, 227, 1)'}}>
                            {testListMock.filter(x => x.disciplineId === props.id && x.checked === props.isChecked).map(x =>
                                <TableRow>
                                    <TableCell>{x.title}</TableCell>
                                    <TableCell>{x.correctAnswers || 0}/{x.taskCount}</TableCell>
                                    {props.isChecked ? <TableCell>{x.mark}</TableCell> :
                                        <TableCell><Link to={`/test/${x.id}`}>Выполнить</Link></TableCell>
                                    }
                                </TableRow>
                            )}
                        </TableBody>
